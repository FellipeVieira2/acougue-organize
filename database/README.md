...
Execução: `pnpm test:integration`. Requer PostgreSQL 17 e privilégios de criação de role/schema no banco de teste. Sem URL, o teste falha explicitamente; não é ignorado. O runner versionado aplica as quatro migrations com checksum e lock transacional, sem permitir que o BEGIN/COMMIT da fundação encerre a transação externa. A suíte também verifica rollback de instalação, reaplicação concorrente, alteração de checksum, proprietário inicial e precisão de preços.
