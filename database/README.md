...
A migration 0001 cria organização, loja, produto, revisão de preço e auditoria com RLS, FKs compostas e role de runtime sem login e sem bypass. A migration 0005 adiciona a primeira fatia do modelo de açougue: preparação, item físico de estoque, oferta, saldo, movimentos e reservas, também com RLS. A migration 0006 adiciona pedidos, itens com snapshot, eventos e ligação explícita das reservas ao pedido. A migration 0007 adiciona a resolução controlada de lojas públicas por slug. A migration 0008 adiciona idempotência de checkout e hash de token público de acompanhamento. Billing e interface de acompanhamento ainda não fazem parte do banco.
