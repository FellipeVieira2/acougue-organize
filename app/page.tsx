  ...
  return <main className="app-shell">
    ...
    <section className="main-content">
      ...
      <div className="content-wrap">
        ...
        {active === "Pedidos" ? <OrderQueue key={selectedStore?.id ?? "none"} orders={data.orders} onChanged={() => mutate()} /> : active === "Estoque" ? <section className="table-card empty-state"><Boxes /><h2>Controle de estoque em preparação</h2><p>Movimentações, saldos e alertas estarão disponíveis em uma próxima etapa.</p></section> : active === "Lojas" ? <section className="table-card"><div className="section-heading"><h2>Suas lojas</h2></div>{data.stores.map(store => <div className="activity-row" key={store.id}><Store /><strong>{store.name}</strong><span className={store.active ? "badge badge-green" : "badge badge-gray"}>{store.active ? "Ativa" : "Inativa"}</span>{store.active && <StoreLink slug={store.slug} />}</div>)}</section> : <>
