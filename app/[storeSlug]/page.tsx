...
function money(minor: string) {
  return new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(minor) / 100)
}
...
export default function Storefront({ params }: { params: Promise<{ storeSlug: string }> }) {
  ...
  const estimatedTotal = cart.reduce((sum, line) => sum + Number(line.offer.amountMinor) * (line.offer.saleUnit === "G" ? Number(line.quantity) / 1000 : Number(line.quantity)), 0)
  ...
  function add(offer: Offer) {
    setCart(current => {
      ...
      return current.map(line => line.offer.id === offer.id ? { ...line, quantity: String(Number(line.quantity) + Number(offer.saleUnit === "G" ? offer.weightStepG ?? "500" : "1")) } : line)
    })
    setCartOpen(true)
  }
  function change(offerId: string, delta: number) {
    setCart(current => current.flatMap(line => {
      ...
      const minimum = Number(line.offer.minWeightG ?? (line.offer.saleUnit === "G" ? "500" : "1"))
  ...
  return <main className="storefront-shell">
    ...
    <section className="offer-grid">{filteredOffers.map(offer => <article className="offer-card" key={offer.id}><div className="offer-art"><Beef /><span>{offer.saleUnit === "G" ? "por kg" : "por unidade"}</span></div><div className="offer-content"><div className="offer-title"><div><p>{offer.productName}</p><h3>{offer.preparationName}</h3></div><span className="offer-price">{money(offer.amountMinor)}<small>/{offer.saleUnit === "G" ? "kg" : "un."}</small></span></div><p className="offer-range">{formatWeightRange(offer)}</p><button className="offer-button" onClick={() => add(offer)}>Adicionar <Plus /></button></div></article>)}</section>
    ...
    {cartOpen && <div className="drawer-backdrop" onClick={() => setCartOpen(false)}><aside className="cart-drawer" onClick={event => event.stopPropagation()}><div className="drawer-heading"><div><p className="store-section-kicker">Seu pedido</p><h2>Carrinho</h2></div><button className="icon-button" onClick={() => setCartOpen(false)} aria-label="Fechar carrinho"><X /></button></div>{cart.length === 0 ? <div className="cart-empty"><ShoppingBag /><p>Seu carrinho está vazio.</p></div> : <><div className="cart-lines">{cart.map(line => <div className="cart-line" key={line.offer.id}><div><strong>{line.offer.productName}</strong><span>{line.offer.preparationName}</span><small>{displayQuantity(line.quantity, line.offer.saleUnit)} · {money(line.offer.amountMinor)}/{line.offer.saleUnit === "G" ? "kg" : "un."}</small></div><div className="quantity-control"><button onClick={() => change(line.offer.id, -1)} aria-label="Diminuir quantidade"><Minus /></button><b>{line.offer.saleUnit === "G" ? `${(Number(line.quantity) / 1000).toLocaleString("pt-BR")} kg` : line.quantity}</b><button onClick={() => change(line.offer.id, 1)} aria-label="Aumentar quantidade"><Plus /></button><button className="remove-line" onClick={() => remove(line.offer.id)} aria-label="Remover item"><X /></button></div></div>)}</div><div className="cart-summary"><span>Estimativa</span><strong>{money(String(Math.round(estimatedTotal * 100)))}</strong><small>O total final considera o peso separado no balcão.</small><button className="primary-button checkout-button" onClick={() => { setCartOpen(false); setCheckoutOpen(true) }}>Continuar <ArrowRight /></button></div></>}</aside></div>}
    {checkoutOpen && <Checkout catalog={catalog} cart={cart} busy={busy} setBusy={setBusy} result={result} setResult={setResult} onClose={() => setCheckoutOpen(false)} />}
  </main>
}

function Checkout({ catalog, cart, busy, setBusy, result, setResult, onClose }: { catalog: Catalog; cart: CartLine[]; busy: boolean; setBusy: (value: boolean) => void; result: CheckoutResult | null; setResult: (value: CheckoutResult | null) => void; onClose: () => void }) {
  const [error, setError] = useState("")
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setError("")
    const fields = new FormData(event.currentTarget)
    const body = { customerName: fields.get("customerName"), customerPhone: fields.get("customerPhone"), fulfillmentType: fields.get("fulfillmentType"), currency: "BRL", customerNote: fields.get("customerNote") || null, items: cart.map(line => ({ offerId: line.offer.id, requestedQty: line.quantity, customerNote: null })) }
    try {
      const response = await fetch(`/api/public/stores/${encodeURIComponent(catalog.store.slug)}/orders`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify(body) })
