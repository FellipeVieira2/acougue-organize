...
export default function Storefront({ params }: { params: Promise<{ storeSlug: string }> }) {
  ...
  return <main className="storefront-shell">
    ...
    <section className="offer-grid">{filteredOffers.map(offer => <article className="offer-card" key={offer.id}><div className="offer-art"><Beef /><span>{offer.saleUnit === "G" ? "por kg" : "por unidade"}</span></div><div className="offer-content"><div className="offer-title"><div><p>{offer.productName}</p><h3>{offer.preparationName}</h3></div><span className="offer-price">{money(offer.amountMinor, offer.currency)}<small>/{offer.saleUnit === "G" ? "kg" : "un."}</small></span></div><p className="offer-range">{offer.displayOnly ? "O açougue ainda precisa configurar este item para pedidos online." : formatWeightRange(offer)}</p><button className="offer-button" disabled={offer.displayOnly || offer.saleUnit === "FIXED_PACKAGE"} onClick={() => add(offer)}>{offer.displayOnly ? "Consulte a loja" : offer.saleUnit === "FIXED_PACKAGE" ? "Em breve" : "Adicionar"} {!offer.displayOnly && <Plus />}</button></div></article>)}</section>
