...
test('fluxo HTTP: cadastro, isolamento, catálogo, origem e logout', async () => {
  ...
  const id = dashboard.products[0].id;
  const changes = {name:'Patinho atualizado', active:false, expectedVersion:dashboard.products[0].version};
  assert.equal((await request('products/' + id, changes, secondCookie)).status, 404);
  assert.equal((await request('products/' + id, {...changes, actorId:randomUUID()}, cookie)).status, 400);
  assert.equal((await request('products/' + id, changes, cookie, 'https://evil.example.test')).status, 403);
  const concurrent = await Promise.all([request('products/' + id, changes, cookie),request('products/' + id, {...changes,name:'Outra edição'}, cookie)]);
  assert.deepEqual(concurrent.map(result => result.status).sort(), [200,409]);
  const updated = await (await request('dashboard', undefined, cookie)).json();
  assert.equal(updated.products[0].active, false);
  assert.equal(updated.products[0].version, '2');
  assert.equal(updated.products[0].amountMinor, product.amountMinor);
  assert.equal(updated.activity.filter((event: {action:string}) => event.action === 'product.updated').length, 1);
  assert.equal((await request('products/' + id, {...changes,expectedVersion:'2',active:true}, cookie)).status, 200);
  assert.equal((await (await request('dashboard',undefined,cookie)).json()).products[0].active,true);
